...
def reset_game():
    ...
    st.session_state.speech_style = "다정한"   # 기본 말투 스타일
    st.session_state.love_style = "테토"       # 테토 / 에겐
...
def get_style_instruction(style: str) -> str:
    """말투 스타일(다정한, 장난끼많은, 어른스러운, 새침한, 수줍은)에 따른 설명"""
    if style == "다정한":
        return (
            "말투는 전반적으로 따뜻하고 다정하다. "
            "상대방을 배려하는 표현을 자주 사용하고, 말 끝은 부드럽게 처리한다. "
            "상대의 감정을 잘 챙겨주는 스윗한 느낌을 유지하라."
        )
    elif style == "장난끼많은":
        return (
            "말투는 장난끼가 많고 위트가 넘친다. "
            "가벼운 드립이나 놀리는 표현을 적당히 섞어서 분위기를 즐겁게 만든다. "
            "너무 공격적이지 않게, 귀엽게 놀리는 톤을 유지하라."
        )
    elif style == "어른스러운":
        return (
            "말투는 차분하고 성숙하다. "
            "말을 서두르지 않고, 한 번 더 생각한 듯한 깊이 있는 표현을 사용하라. "
            "상대의 이야기를 존중하며, 안정감과 신뢰감을 주는 톤을 유지하라."
        )
    elif style == "새침한":
        return (
            "말투는 약간 새침하고 쉽게 마음을 열지 않는 듯한 느낌을 준다. "
            "너무 친절하게 굴기보다는, 적당히 선을 지키며 살짝 츤데레 같은 반응을 하라. "
            "그러면서도 완전히 차갑지는 않게, 숨겨진 귀여움이 느껴지도록 연기하라."
        )
    elif style == "수줍은":
        return (
            "말투는 수줍고 조심스럽다. "
            "문장 끝에 망설임이 느껴지는 말투나, 부끄러워하는 표현을 자주 사용하라. "
            "직설적으로 말하기보다는 살짝 돌려 말하고, 상대의 반응을 신경 쓰는 모습을 보여라."
        )
    # 혹시 모르니 기본값도 부드러운 톤으로
    return (
        "말투는 자연스럽고 캐주얼하며, 일상 대화처럼 편안하고 부드럽다."
    )


def get_love_style_instruction(style: str, gender: str) -> str:
    """
    테토 / 에겐 연애 스타일 설명 추가
    - 테토: 스스로 해결, 독립적, 쿨
    - 에겐: 애정표현 많고, 상대에게 기대는 스타일
    """
    if style == "테토":
        return (
            "연애 스타일은 '테토'에 가깝다. "
            "문제를 스스로 해결하려 하고, 쉽게 상대에게 기대지 않으며, "
            "독립적이고 자기 주장이 뚜렷한 편이다. "
            "감정 표현도 비교적 담백하고 쿨한 인상을 준다."
        )
    elif style == "에겐":
        return (
            "연애 스타일은 '에겐'에 가깝다. "
            "애정 표현이 많고, 상대에게 조금 의지하고 기대고 싶어하며, "
            "애교 섞인 말투와 솔직한 감정 표현을 자주 사용한다. "
            "상대의 관심과 애정을 확인받고 싶어하는 경향이 있다."
        )
    return ""
...
# 1. 닉네임 / MBTI / 성별 / 나이대 / 말투 / 테토-에겐 선택 단계
if not st.session_state.game_started:
    ...
    st.session_state.speech_style = st.selectbox(
        "상대방의 말투 스타일을 골라주세요:",
        ["다정한", "장난끼많은", "어른스러운", "새침한", "수줍은"],
    )

    st.session_state.love_style = st.radio(
        "상대방의 연애 스타일(테토 / 에겐):",
        ["테토", "에겐"],
        horizontal=True,
    )
    ...
    if st.button("💕 소개팅 시작"):
        if not st.session_state.nickname.strip():
            ...
            love_style_instruction = get_love_style_instruction(
                st.session_state.love_style,
                st.session_state.partner_gender,
            )
